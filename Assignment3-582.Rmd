...
```{r box plot}
All_MAPE <- data.table(MAPE = c(MAPE_lag168, MAPE_lag48, MAPE_linear_reg, model_lr_hourly$MAPE, model_lasso_hourly$MAPE), Model = c('lag 168 naïve approach', 'lag 48 naïve approach', 'LR', paste(0:23, 'Hour_LR', sep = '_'), paste(0:23, 'Hour_Lasso', sep = '_')))

a <- ggplot(aes(y = MAPE), data = All_MAPE) + 
geom_boxplot(fill="lightpink1")+theme_minimal()+labs(title="MAPE Comparison of Different Approaches", 
